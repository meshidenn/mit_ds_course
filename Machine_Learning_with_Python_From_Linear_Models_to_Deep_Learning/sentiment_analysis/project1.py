...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    h = label * (np.dot(feature_vector, current_theta) + current_theta_0)
    if h <= 0:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    m = n*T
    return(sum_theta/m, sum_theta_0/m)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    predicts = np.matmul(feature_matrix, theta) + theta_0
    labels = np.array([1 if x > 0 else -1 for x in predicts])
    return labels
...
def count_bow_feature_vectors(reviews, dictionary):
    """
    Inputs a list of string reviews
    Inputs the dictionary of words as given by bag_of_words
    Returns the bag-of-words feature matrix representation of the data.
